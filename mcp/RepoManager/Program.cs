    ...
    class Program
    {
        ...
        static async Task Main(string[] args)
        {
            ...
            app.MapGet("/sse", async (HttpContext context) =>
            {
                ...
                // 发送初始的 endpoint 事件，告诉客户端通过哪个 URL POST 消息
                ...
                try
                {
                    // 持续从 Channel 拉取消息并推送给客户端，直到客户端断开
            ...
            app.MapPost("/message", async (HttpContext context, string sessionId) =>
            {
                ...
                // 官方 MCP SSE 标准：收到消息返回 HTTP 202 Accepted
                return Results.Accepted();
            });

            Console.WriteLine("=================================================");
            Console.WriteLine("🚀 RepoManager MCP Server (HTTP/SSE 模式) 正在启动");
            Console.WriteLine("=================================================");
            Console.WriteLine("监听地址: http://localhost:5000");
            Console.WriteLine("SSE 连接端点: http://localhost:5000/sse");
            Console.WriteLine("所有交互日志将会在此控制台高亮实时打印。");
            Console.WriteLine("=================================================");
        ...
        static async Task HandleRequest(string method, JsonElement root, JsonElement? id, Channel<string> channel)
        {
                ...
                switch (method)
                {
                    case "initialize":
                        responseResult = new
                        {
                            ...
                            serverInfo = new { name = "RepoManager", version = "1.0.0" }
                        };
                        break;
                    ...
                    case "tools/call":
                        ...
                        if (toolName == "acquire_lock")
                        {
                            ...
                            responseResult = new { content = new[] { new { type = "text", text = AcquireLock(filepath, agentId) } } };
                        }
                        else if (toolName == "release_lock")
                        {
                            var filepath = argsEl.GetProperty("filepath").GetString()!;
                            var agentId = argsEl.GetProperty("agent_id").GetString()!;
                            responseResult = new { content = new[] { new { type = "text", text = ReleaseLock(filepath, agentId) } } };
                        }
                        else if (toolName == "check_status")
                        {
                            responseResult = new { content = new[] { new { type = "text", text = CheckStatus() } } };
                        }
                        else
                        {
            ...
            if (id.HasValue && (responseResult != null || errorResult != null))
            {
                ...
                {
                    { "jsonrpc", "2.0" },
                    { "id", id.Value.ValueKind == JsonValueKind.Number ? id.Value.GetInt32() : id.Value.GetString()! }
                };

                if (errorResult != null) response["error"] = errorResult;
                else response["result"] = responseResult;

                var jsonStr = JsonSerializer.Serialize(response, new JsonSerializerOptions { PropertyNamingPolicy = JsonNamingPolicy.CamelCase });
        ...
        static string AcquireLock(string filepath, string agentId)
        {
            if (LockedFiles.TryAdd(filepath, agentId))
                return $"Success: [{filepath}] has been locked by [{agentId}]. You can now edit it.";
            
            LockedFiles.TryGetValue(filepath, out var owner);
            if (owner == agentId) return $"Success: You ([{agentId}]) already hold the lock for [{filepath}].";
            return $"Failed: [{filepath}] is currently locked by another agent ([{owner}]). Please wait or work on something else.";
        }
        ...
        static string ReleaseLock(string filepath, string agentId)
        {
            if (LockedFiles.TryGetValue(filepath, out var owner) && owner == agentId)
            {
                ...
                return $"Success: Lock for [{filepath}] has been released.";
            }
            return $"Failed: You cannot release [{filepath}] because you are not the owner. (Current owner: {owner ?? "None"})";
        }

        static string CheckStatus()
        {
            if (LockedFiles.IsEmpty) return "No files are currently locked.";
            var status = new List<string>();
            foreach (var kvp in LockedFiles)
                status.Add($"- {kvp.Key} (Locked by: {kvp.Value})");
            return string.Join("\n", status);
        }
        ...
        static async Task RunStdioBridgeAsync(string serverBaseUrl)
        {
            ...
            try
            {
                using var request = new System.Net.Http.HttpRequestMessage(System.Net.Http.HttpMethod.Get, $"{serverBaseUrl}/sse");
                ...
                while ((inputLine = await stdinReader.ReadLineAsync()) != null)
                {
                    ...
                    while (endpointUrl == null) await Task.Delay(50); // 等待握手完成分配端点
                    
                    var content = new System.Net.Http.StringContent(inputLine, System.Text.Encoding.UTF8, "application/json");
                    await client.PostAsync($"{serverBaseUrl}{endpointUrl}", content);
                }
            }
            catch (Exception ex)
            {
                // Bridge crash, silently exit or write to stderr so it doesn't break JSON-RPC
